...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('extension.createStyledComponent', execute);
...
const execute = (args: any) => {
	...
	const featureSelection: vscode.QuickPickItem[] = [
		{label: CONSTANTS.features.ts, picked: true},
		{label: CONSTANTS.features.storybook, picked: true},
		{label: CONSTANTS.features.tests, picked: true},
	];

	const featureSelectionOptions:vscode.QuickPickOptions & {canPickMany: true} = {
		canPickMany: true,
	...
	vscode.window.showQuickPick<vscode.QuickPickItem>(featureSelection, featureSelectionOptions).then(selectedFeatures => {
		vscode.window.showInputBox(componentNameOptions).then(componentName => {
			...
			if (typeof componentName === "string") {
				...
				try {
					...
					buildReactTemplate(selectedFeatures?.map(s => s.label), componentName, componentFolder);
				}
				catch (error) {
